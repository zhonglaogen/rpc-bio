...
public class Gziputils {
     ...
     */
    @Test
    public void junitTest1() throws Exception {
        FileInputStream fis=new FileInputStream("/home/zhonglianxi/b.jpg");
        byte[] temp=new byte[fis.available()];
        int length=fis.read(temp);
        System.out.println("length::"+length);
        System.out.println();

        byte[] zip = Gziputils.zip(temp);
        System.out.println("zip---->>length"+zip.length);

        byte[] unzip = Gziputils.unzip(zip);
